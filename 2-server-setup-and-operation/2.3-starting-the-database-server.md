...
ระบบปฏิบัติการที่ต่างกัน มีข้อกำหนด (วิธีการ) ในการสั่งให้ `daemon` เริ่มทำงานตอนเปิดเครื่อง (Boot) ที่แตกต่างกันออกไป หลายระบบใช้ไฟล์ `/etc/rc.local` หรือ `/etc/rc.d/rc.local` บางระบบใช้ไดเร็กทอรี `init.d` หรือ `rc.d` ไม่ว่าคุณจะทำอย่างไร เซิร์ฟเวอร์จะต้องทำงานโดยบัญชีผู้ใช้ `postgres` ไม่ใช่โดย `root` หรือผู้ใช้อื่นๆ ตัวอย่างสำหรับ linux:

```
ubuntu:~$ sudo cp postgresql-17.9/contrib/start-scripts/linux /etc/init.d/postgresql
ubuntu:~$ sudo chmod 755 /etc/init.d/postgresql
```
...
# แสดงรายการ log ของ postgresql
ubuntu:~$ sudo journalctl -eu postgresql
```
